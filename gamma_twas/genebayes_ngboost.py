...
from ngboost.distns import MultivariateNormal, Normal, k_categorical, RegressionDistn
from ngboost.learners import default_tree_learner
from ngboost.manifold import manifold
from ngboost.scores import LogScore


class GeneBayesNGBoost:
    """
    ...
    def sample(self, X, Y, F, sample_weight, params):
    ...
    def line_search(self, resids, start, Y, F, sample_weight=None, scale_init=1):
        D_init = self.Manifold(start.T, F.T)
        ...
        while True:
            ...
            D = self.Manifold((start - scaled_resids).T, F.T)
    ...
    def partial_fit(
        ...
        if self.early_stopping_rounds is not None:
            ...
            if sample_weight is None:
                X, X_val, Y, Y_val, F, F_val = train_test_split(
                    X,
                    Y,
                    F,
                ...
                X, X_val, Y, Y_val, F, F_val, sample_weight, val_sample_weight = train_test_split(
                    X,
                    Y,
                    F,
                    sample_weight,
        ...
        assert len(F.shape) == 2, 'Fixed parameters must be a matrix of two dimensions'
        assert F.shape[0] == Y.shape[0], 'Fixed parameters must match the length of the response and features'
        ...
        if X_val is not None and Y_val is not None and F_val is not None:
            ...
            assert len(F_val.shape) == 2, 'Fixed parameters must be a matrix of two dimensions'
            assert F_val.shape[0] == Y_val.shape[0], 'Fixed parameters must match the length of the response and features'
        ...
        for itr in range(len(self.col_idxs), self.n_estimators + len(self.col_idxs)):
            _, col_idx, X_batch, Y_batch, F_batch, weight_batch, P_batch = self.sample(
                X, Y, F, sample_weight, params
            )
            self.col_idxs.append(col_idx)

            D = self.Manifold(P_batch.T, F_batch.T)
            ...
            scale = self.line_search(proj_grad, P_batch, Y_batch, F_batch, weight_batch)
            ...
            if X_val is not None and Y_val is not None and F_val is not None:
                ...
                val_loss = val_loss_monitor(self.Manifold(val_params.T, F_val.T), Y_val)
    ...
    def score(self, X, Y, F):  # for sklearn
        return self.Manifold(self.pred_dist(X)._params, F).total_score(Y)

    def pred_dist(self, X, F, max_iter=None):
        """
        ...
        if (
            ...
            dist = self.staged_pred_dist(X, F, max_iter=max_iter)[-1]
        else:
            params = np.asarray(self.pred_param(X, max_iter))
            dist = self.Dist(params.T, F.T)
        return dist

    def staged_pred_dist(self, X, F, max_iter=None):
        """
    ...
    def predict(self, X, F, max_iter=None):
        """
        ...
        return self.pred_dist(X, F, max_iter=max_iter).predict()

    def staged_predict(self, X, F, max_iter=None):
        """
        ...
        """
        return [dist.predict() for dist in self.staged_pred_dist(X, F, max_iter=max_iter)]
...
class GeneBayesNGBRegressor(GeneBayesNGBoost, BaseEstimator):
    """
