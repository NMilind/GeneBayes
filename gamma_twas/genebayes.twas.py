...
U = 2 * X + X * np.random.randn(1000)
Y_s = np.random.chisquare(1, 1000)
Y = (Y_s * np.random.randn(1000)) + U
Y = np.vstack((Y, Y_s)).T
X = X.reshape(-1, 1)
...
ax.scatter(X_train.flatten(), Y_train[:, 0])
ax.scatter(X_test.flatten(), Y_test[:, 0])
...
def main():
    ...
    model = NGBRegressor(
        n_estimators=500, Dist=Prior, Base=learner, Score=PriorScore,
        verbose_eval=1, learning_rate=0.05, natural_gradient=True
    ).fit(X_train, Y_train, X_val=X_test, Y_val=Y_test, early_stopping_rounds=10)
